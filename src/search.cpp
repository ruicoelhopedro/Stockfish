...
namespace {
  ...
  constexpr int futility_move_count(bool improving, Depth depth, bool nullMove) {
    return (3 + depth * depth) / (2 - improving) / (1 + nullMove);
  }
...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
          ...
          moveCountPruning = moveCount >= futility_move_count(improving, depth, (ss-1)->currentMove == MOVE_NULL);
