...
namespace {
  ...
  // Futility margin
  Value futility_margin(Depth d, bool improving, int moveCount) {
    return Value(168 * (d - improving) + 100 / (moveCount + 1));
  }
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (   !ss->ttPv
        &&  depth < 8
        &&  eval - futility_margin(depth, improving, (ss-1)->moveCount) - (ss-1)->statScore / 256 >= beta
        &&  eval >= beta
